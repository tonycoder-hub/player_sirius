...
- 已支持 SMB 连接配置输入、配置保存和 `libsmb2` 依赖占位接入口。
...
- 当前 Native 桥接会明确暴露 `backendName/blocker`，用于标识真实软解内核的剩余阻塞点。
...
- 后台播放与音频焦点接入
- 外挂字幕真实渲染与多音轨真实切换
- 真正接入 `FFmpeg/libav*` 或等效软解依赖，打通实际 `demux / decode / render / A/V sync`
...
3. 继续增强自动化回归与真机验证，覆盖 DLNA、媒体库增量重扫和 Native 桥接状态。
